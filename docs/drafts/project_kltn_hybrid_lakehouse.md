...
Synthetic source data is stored under `master_data/raw/` as `transcript_batch*.json` files.

---

## Mock Data and Normalization

`master_data/raw/` contains thousands of transcript batch JSON files covering telesales calls over several days in March 2026.

Important normalization logic lives in:
- `master_data/split_to_entities.py`

What it does:
- loads all `raw/transcript_batch*.json`
